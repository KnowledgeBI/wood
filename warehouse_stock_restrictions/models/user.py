...
class ResUsers(models.Model):
    ...
    sale_warehouse_id = fields.Many2one(comodel_name="stock.warehouse", string="Default Warehouse")
    ...
    def update_restrict(self):
        restrict_group = self.env.ref('warehouse_stock_restrictions.stock_restrictions_group')
    ...
    def tgl_restrict(self):
        ...
        if self.stock_location_ids:
            ...
            pick_types = self.env['stock.picking.type'].sudo().search(['|', '|', '|',
                                                                       ('default_location_src_id', 'in',
                                                                        [l.id for l in self.stock_location_ids]),
                                                                       ('default_location_src_id.location_id', 'in',
                                                                        [l.id for l in self.stock_location_ids]),
                                                                       ('default_location_dest_id', 'in',
                                                                        [l.id for l in self.stock_location_ids]),
                                                                       ('default_location_dest_id.location_id', 'in',
                                                                        [l.id for l in self.stock_location_ids]),
                                                                       ])
            current_group.write({'users': [(4, self.id)]})
            self.groups_id = [(4, restrict_group.id)]
